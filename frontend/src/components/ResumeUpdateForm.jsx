...
const ResumeUpdateForm = ({ resume }) => {
  ...
  const handleAddWorkExperience = () => {
    ...
    setFormData({
      ...formData,
      workExperiences: [...formData.workExperiences, newWorkExperience],
    })
  }
  ...
  const handleDeleteWorkExperience = (e) => {
    const updatedWorkExperiences = formData.workExperiences.filter((w) => {
      if (w.companyName && w.startDate) {
        return (
          w.companyName !== e.target.dataset.companyName &&
          w.startDate !== e.target.dataset.startDate
        )
      } else if (w.id) {
  ...
  const handleDeleteEducation = (e) => {
    const updatedEducations = formData.educations.filter((edu) => {
      if (edu.institution && edu.startDate) {
        return (
          edu.institution !== e.target.dataset.institution &&
          edu.startDate !== e.target.dataset.startDate
        )
      } else if (edu.id) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
            ...
            <div key={w.id} className="border p-4 rounded-lg mb-4">
              <div className="flex justify-between">
                ...
                <button
                  ...
                  data-company-name={w.companyName}
                  data-start-date={w.startDate}
                  id={w.id}
                >
                  Delete
                </button>
              ...
              <div>
                ...
                <input
                  ...
                  name={`${w.id}${w.companyName}`}
                  id={`${w.id}${w.companyName}`}
                  value={w.companyName}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      workExperiences: formData.workExperiences.map((we) =>
                        we.id === w.id
                          ? { ...we, companyName: e.target.value }
                          : we
                      ),
                    })
                  }
              ...
              <div>
                ...
                <input
                  ...
                  name={`${w.id}${w.responsibility}`}
                  id={`${w.id}${w.responsibility}`}
                  value={w.responsibility}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      workExperiences: formData.workExperiences.map((we) =>
                        we.id === w.id
                          ? { ...we, responsibility: e.target.value }
                          : we
                      ),
                    })
                  }
        ...
        <button
          type="button"
          className="text-violet-800 flex items-center gap-1"
        ...
        {formData.educations.map((edu, i) => {
          return (
            <div key={edu.id} className="border p-4 rounded-lg mb-4">
              ...
              <div>
                <label
                  htmlFor={`${edu.id}${edu.institution}`}
                  className="block mb-2 text-gray-500"
                >
                  Institution
                </label>
                <input
                  ...
                  name={`${edu.id}${edu.institution}`}
                  id={`${edu.id}${edu.institution}`}
              ...
              <div className="flex justify-between gap-4">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full mb-4 p-2 border rounded"
                    placeholder="Start Date"
                    name={`${edu.id}${edu.startDate}`}
                    id={`${edu.id}${edu.startDate}`}
                    value={edu.startDate}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    name={`${edu.id}${edu.endDate}`}
                    id={`${edu.id}${edu.endDate}`}
                    value={edu.endDate}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
